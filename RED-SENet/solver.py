...
from se_networks_new import RED_CNN
...
class Solver(object):
    ...
    def save_model(self, iter_):
        f = os.path.join(self.save_path, 'REDCNN_{}_iter.ckpt'.format(iter_))
        torch.save(self.REDCNN.state_dict(), f)


    def load_model(self, iter_,args):  # 测试的时候需要加载训练时候保存的模型
        f = os.path.join(self.save_path, 'REDCNN_{}_iter.ckpt'.format(iter_))
        if self.multi_gpu:
            state_d = OrderedDict()
            pretrained_dict = torch.load(f)
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if np.shape(sata_d[k]) == np.shape(v)}
            # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in state_d}
            state_d.update(pretrained_dict)
            # for k, v in torch.load(f):
            #     n = k[7:]
            #     state_d[n] = v
            self.REDCNN.load_state_dict(state_d)
        else:
            # sata_d = self.REDCNN.state_dict()
            pretrained_dict = torch.load(f)
            # pretrained_dict = {k: v for k, v in pretrained_dict.items() if np.shape(sata_d[k]) == np.shape(v)}
            # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in state_d}
            # state_d.update(pretrained_dict)
            self.REDCNN.load_state_dict(pretrained_dict)
    ...
    def train(self, args):
        ...
        for epoch in range(1, self.num_epochs):
            ...
            for iter_, (x, y) in enumerate(self.data_loader):
                ...
                loss.backward()
                self.optimizer.step()
                train_losses.append(loss.item())  # item()用于将一个零维张量转换成浮点数，如计算loss，accuracy
                # append()方法向列表尾部添加一个新的“元素”，只接受一个参数
                # extend()方法只接受一个“列表”作为参数，并将该参数的每个元素都添加到到原有的参数列表中

                # print
                if total_iters % self.print_iters == 0:
                    print("STEP [{}], EPOCH [{}/{}], ITER [{}/{}] \nLOSS: {:.8f}, TIME: {:.1f}s, LR: {:.8f}".format(total_iters, epoch,
    ...
    def test(self,args):
        ...
        self.load_model(self.test_iters,args)
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(self.data_loader):
                shape_ = x.shape[-1]   # x 的最后一个维度，宽和高
                x = x.unsqueeze(0).float().to(self.device)
                y = y.unsqueeze(0).float().to(self.device)
